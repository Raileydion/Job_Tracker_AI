...
function Spinner({ color = "#c9a84c", size = 20 }: { color?: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none"
      style={{ animation: "trackrSpin 0.8s linear infinite" }}>
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase.from("jobs").select("status").eq("user_id", sd.session.user.id);
  ...
  if (loading) return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 14, fontFamily: "'Outfit', sans-serif" }}>
      ...
      <span style={{ color: txM, fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase" }}>Loading analytics</span>
    </div>
  );
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        .ch{transition:border-color 0.2s,box-shadow 0.2s,transform 0.18s;}
        .ch:hover{border-color:${goldB}!important;transform:translateY(-2px);box-shadow:0 8px 28px rgba(0,0,0,0.3)!important;}
      ...
      <div style={{ padding: "32px 28px 64px", display: "flex", flexDirection: "column", gap: 28, fontFamily: "'Outfit', sans-serif", color: tx }}>

        {/* Header */}
        <div className="sup">
          <p style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.14em", color: txM, marginBottom: 8 }}>Overview</p>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 30, fontWeight: 400, color: tx, letterSpacing: "0.01em", lineHeight: 1.1, marginBottom: 6 }}>
            Analytics
          </h1>
        ...
        <div className="sg4" style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(4,1fr)" }}>
          {STAT_CARDS.map((s, i) => (
            <div key={s.label} className="sup ch" style={{
              background: bgCard, border: `1px solid ${brd}`, borderRadius: 16,
              ...
              <p style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.13em", color: txM, marginBottom: 16 }}>{s.label}</p>
              ...
              <p style={{ fontSize: 11, color: txF }}>{s.sub}</p>
            </div>
          ))}
        </div>
        ...
        <div className="sg2" style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 14 }}>
          ...
          {/* Funnel */}
          <div className="sup ch" style={{ background: bgCard, border: `1px solid ${brd}`, borderRadius: 16, padding: "24px 24px 22px", animationDelay: "240ms" }}>
            <p style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.13em", color: txM, marginBottom: 22 }}>Conversion Funnel</p>
            {stats.total === 0 ? (
              <p style={{ textAlign: "center", padding: "28px 0", color: txF, fontSize: 13 }}>Add applications to see your funnel</p>
            ) : (
                    ...
                    <div key={f.label}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <span style={{ fontSize: 12.5, color: txM }}>{f.label}</span>
                        </div>
                        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                          {conv !== null && <span style={{ fontSize: 10, color: txF }}>{conv}% from prev</span>}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {RATE_CARDS.map((r, i) => (
              <div key={r.label} className="sup ch" style={{
                background: bgCard, border: `1px solid ${brd}`, borderRadius: 14,
                ...
                <div>
                  <p style={{ fontSize: 13.5, fontWeight: 500, color: tx, marginBottom: 4 }}>{r.label}</p>
                  <p style={{ fontSize: 12, color: txM, fontWeight: 300, lineHeight: 1.5 }}>{r.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* ── STATUS BREAKDOWN ── */}
        <div className="sup ch" style={{ background: bgCard, border: `1px solid ${brd}`, borderRadius: 16, padding: "24px 24px 22px", animationDelay: "400ms" }}>
          <p style={{ fontSize: 10, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.13em", color: txM, marginBottom: 20 }}>Status Distribution</p>
          {stats.total === 0 ? (
            <div style={{ textAlign: "center", padding: "24px 0" }}>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: txM, marginBottom: 6 }}>No data yet</p>
              <p style={{ color: txF, fontSize: 13 }}>Start adding applications to visualise your pipeline</p>
            </div>
          ) : (
            <>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px 24px" }}>
                {[
                  ...
                  <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <span style={{ fontSize: 12, color: txM }}>{s.label}</span>
                    <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, color: tx }}>{s.count}</span>
                    <span style={{ fontSize: 11, color: txF }}>({Math.round((s.count / stats.total) * 100)}%)</span>
                  </div>
                ))}
              </div>
            </>
